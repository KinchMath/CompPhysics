...
vxj0 = np.sqrt(G*M*(2/Aphelion2-1/semimajor2))
...
for i in range (posorb[0],len(xesol),delta):
    ...
    print('Orbit {} Aphelion value:\t '.format(year),'{:.3}'.format(R[myfavourite+aposition]))
    
    #perihelion return
    periposition=np.argmin(R[myfavourite:i+1])
    print('\t\tPerihelion value:','{:.3}\n'.format(R[myfavourite+periposition]))
...
print('Initial Semi-Major Axis: {:.3}'.format(semib))


Rf=np.sqrt(xorb2**2+yorb2**2)
semif=(Rf[np.argmax(Rf)]+Rf[np.argmin(Rf)])/2
print(Rf[np.argmax(Rf)],Rf[np.argmin(Rf)])
print('\nFinal Semi-Major Axis: {:.3}'.format(semif)) # double check doesnt seem completely right
...
print('\nInitial Eccentricity: {:.3}'.format((Rb[np.argmax(Rb)]-Rb[np.argmin(Rb)])/(Rb[np.argmax(Rb)]+Rb[np.argmin(Rb)])))
print('Final Eccentricity: {:.3}'.format((Rf[np.argmax(Rf)]-Rf[np.argmin(Rf)])/(Rf[np.argmax(Rf)]+Rf[np.argmin(Rf)])))
...
def animate(i):
    line.set_xdata(xorb[i])
    line.set_ydata(yorb[i])  
    return line,

error might be of 3 sig fig because of max step size
""" 